    ...
    const cUser = useStore(state => state.cUser)
      ...
      "July", "August", "September", "October", "November", "December"
        ...
        const cancelReq = async () => {
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  setloading(true)
                    try{
                        const response= await Axios.patch(`https://newback19.onrender.com/appointment`,
                        {
        ...
        const accept = async () =>{  
          Swal.fire({
            ...
            if (result.isConfirmed) {
              setloading(true)
                try{
                    const response= await Axios.post(`https://newback19.onrender.com/getEvents`,
                    {
                      content:appointmentDetails,
        ...
        const remove = async () =>{  
            ...
            //   if (result.isConfirmed) {
            //       try{
            //           const response= await Axios.delete(`https://newback19.onrender.com/appointment/${appointmentDetails.requestID}`)
            //           getAppointmentReq()
            //           setresponse(false)
            //              Swal.fire({
            //                 icon: 'success',
